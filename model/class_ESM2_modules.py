...
import sys
from .class_ESM2_attention import ColumnSelfAttention, RowSelfAttention, MultiheadAttention
...
class ConservationPooling(nn.Module):
    """Mask-protected conservation pooling"""
    def __init__(self, base_size):
        super().__init__()
        self.cons_predictor = nn.Sequential(
            nn.Linear(base_size, 128),
            nn.ReLU(),
            nn.Linear(128, 1),
            nn.Sigmoid()
        )
    ...
    def forward(self, sequence_output, mask):
        """
        sequence_output: [B, T, E]
        mask: [B, T] (1, 0)
        """
        cons_raw = self.cons_predictor(sequence_output).squeeze(-1)  # [B, T]
        cons_masked = cons_raw * mask.float()
        weights = (0.5 + 1.5 * cons_masked).unsqueeze(-1)
        masked_features = sequence_output * mask.unsqueeze(-1).float()
        numerator = (masked_features * weights).sum(dim=1)  # [B, E]
        denominator = weights.sum(dim=1)  # [B, 1]
        
        return numerator / (denominator + 1e-8)


def MeanPooling(chain_out, mask):
    ...
    return mean_chain_out  # [B,1280]


class LinearConstract(nn.Module):
...
class EsmClassificationHead(nn.Module):
    """ESM sequence classification head module"""
    def __init__(self, hidden_dim=640, num_labels=2):
        super().__init__()
    ...
    def forward(self, features):
        x = self.dense(features)
        x = torch.tanh(x)  # Hyperbolic tangent activation
        x = self.dropout(x)
