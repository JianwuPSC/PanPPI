...
def Graph_constract(df,split_threshold=2,resolution=2):
    ...
    Returns:
        tuple: Four DataFrames for training, validation, test, and cross-set interactions
    """
    ...
    target_ratios = {'train': 0.6, 'valid': 0.2, 'test': 0.2}
    current_sizes = {'train': 0, 'valid': 0, 'test': 0}
    ...
    print(f"train: {len(train_df)/total:.1%} (train: 60%)")
    print(f"valid: {len(valid_df)/total:.1%} (valid: 20%)")
    print(f"test: {len(test_df)/total:.1%} (test: 20%)")

    return train_df, valid_df, test_df, cross_df

# Command line argument parsing
posi_path = sys.argv[1] #" 2711_eukaryotes_min900_redun.txt"

train_out = sys.argv[2] # h5_embedding/zz.train.csv
valid_out = sys.argv[3] # h5_embedding/zz.valid.csv
test_out = sys.argv[4] # h5_embedding/zz.test.csv
cross_out = sys.argv[5] # h5_embedding/zz.cross.csv

df  = pd.read_table(posi_path,names=['protein1',"protein2","label"])

# Partition the dataset using graph-based method
train_df, valid_df, test_df, cross_df = Graph_constract(df,split_threshold=2,resolution=2)
